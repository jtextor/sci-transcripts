...
You may have encountered linear regression in various Bachelor courses, including "Data Analysis", "Data Mining", and "AI". All of these cover some of the topics we'll cover here in more depth. I'll try not to repeat too much, but I'll go through some of the basic materials we'll need in the beginning
...
## Some example normal distributions
...
There's another useful way to distinguish two models of the type 
 $P(x,y)$ and $P(y \mid x)$, which we'll talk about now.
...
For example, when we're trying to build a classifier, we would typically reach for a model of this second kind – such as a logistic regression model – rather than of the first kind (e.g., Naive Bayes).
...
For instance, if I have a classifier of cats versus dogs, I can't use that to make new pictures of cats and dogs.
...
It’s possible in theory to build an AI that detects ChatGPT-written reports by training a classifier on latent representations of AI vs. human text. But in practice it wouldn’t be reliable because the classifier operates in what we call an _adversarial_ context: students will actively adjust their text until it passes. AI models are generally very vulnerable to such attacks. In image classification models, for instance, adversarially changing just a few pixels could make a pig get classified as an airplane. Therefore, while AI detectors can be built in principle, they won't be reliable as they are too easy to fool.
...
Approximately 34%. Now let's verify that this matches what we get by integrating the density function $${\cal{N}(1,1)}$$. We can't do that analytically, but in R we have the integral available using the function `pnorm`. For example, we can get the integral 
...
It's an assumption, yes, but there's generally no reason to believe a normal distribution works best. In fact, we can also make a regression model with Laplace errors, Cauchy distributed errors etc. But when we talk about "the" regression model,
...
called the squared error, so let's start with that. Loss functions are often written using $L$, and they are a function of my prediction and the actual observation. For example, the _squared loss_ is 
...
$$\frac{\delta}{\delta \mu}\sum_{i=1}^n| x_i - \mu |=
\sum_{i=1}^n \text{sign}(x_i - \mu) $$
